...
(function () {
   ...
   * @param data training data in ARFF js format (node-arff)
    ...
    async.waterfall([
      function (callback) {
        child = exec('java -classpath api/public/weka.jar ' + options.classifier + ' '
          ...
          ' -d ' + model,
          // ' -T ' + test + 
          //'-split-percentage 90 -d api/public/newmodel.model',
          function (error, stdout, stderr) {
  ...
  /**
   * Load model and classify the test item.
   * @param model Data in ARFF js format (node-arff)
